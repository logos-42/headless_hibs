...
启动:
    ...
    streamlit run twistor_studio/app.py

或者直接:
    python -m twistor_studio
"""
import os
import sys
import time
    ...
    if not st.session_state.data_files:
        ...
        with col1:
            if st.button("🔄 转换为训练格式", use_container_width=True):
                with st.spinner("转换中..."):
                    from twistor_studio.importer import convert_to_training_format
...
def page_train():
    ...
    if st.button("🚀 开始训练", type="primary", use_container_width=True):
        ...
        with st.spinner("训练中... 请勿关闭页面"):
            from twistor_studio.trainer import train_personal_model
...
def page_chat():
    ...
    def load_engine(path):
        from twistor_cli.inference import InferenceEngine
...
def page_export_mobile():
    ...
    if st.button("📦 生成手机包", use_container_width=True):
        with st.spinner("导出中..."):
            from twistor_studio.exporter import export_mobile_package
...
# ============================================================
def main():
    """CLI 入口: python -m twistor_studio"""
